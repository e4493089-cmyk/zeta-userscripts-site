...
function getBookmarklets(){
  ...
  return {
    ...
    roomIos:"javascript:(async()=>{const k=\"__zetaRoomManagerIosBookmarkletV2\",u=\"https://raw.githubusercontent.com/e4493089-cmyk/zeta-userscripts/main/zeta-room-manager-ios.user.js\",n=\"Room Manager (iOS)\",v=\"0.20.53\";if(location.hostname!==\"zeta-ai.io\"&&!location.hostname.endsWith(\".zeta-ai.io\"))return alert(\"제타 페이지에서 실행해줘\");if(window.__zrmRoomManagerIosVersion===v)return alert(n+\" \"+v+\" 이미 실행 중\");try{window[k]=1;const r=await fetch(u+\"?bm=\"+Date.now(),{cache:\"no-store\"});if(!r.ok)throw Error(\"HTTP \"+r.status);(0,eval)(await r.text());alert(n+\" \"+(window.__zrmRoomManagerIosVersion||v)+\" 적용됨\")}catch(e){delete window[k];alert(n+\" 실행 실패: \"+e.message)}})();",
