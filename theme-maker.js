const BASE_THEME_VERSION = '3.50.1';
const BASE_THEME_URL = 'https://raw.githubusercontent.com/e4493089-cmyk/zeta-userscripts/3ab89847565202bf241b8ba49adb3c4d8c662334/zeta-kakaotalk-theme.user.js';
...
function themeRuntime(A, O, C, F, B, U, notify) {
  ...
  const custom=()=>{const hov=lum(A)>.55?mx(A,'#000000',.08):mx(A,'#FFFFFF',.1),surf=lum(C)<.28?mx(C,'#FFFFFF',.86):mx('#FFFFFF',C,.025),s2=mx(surf,C,.08),line=mx(surf,txt(surf),.1),sub=mx(txt(surf),C,.45),me=txt(A),ai=txt(O),aiAction=mx(ai,O,.34),aiSoft=mx(O,ai,.08),aiLine=mx(O,ai,.16),meBorder=mx(A,me,.18),otherBorder=mx(O,ai,.18);return '\n\n/* Zeta Theme Maker custom overrides */'
...
function buildStandaloneScript() {
  const A=accentPicker.value.toUpperCase(), O=otherPicker.value.toUpperCase(), C=bgPicker.value.toUpperCase(), F=getFontSize(), B=!!bubbleBorder?.checked;
  return `// ==UserScript==
// @name         Zeta Custom Theme (${A} · ${O} · ${C} · ${F}px)
// @namespace    zeta-custom-theme-maker
// @version      1.2.7
// @description  Zeta Theme Maker 생성본 · 내 말풍선 ${A} · 캐릭터 말풍선 ${O} · 배경 ${C} · 글씨 ${F}px · 외곽선 ${B?'ON':'OFF'}
// @match        https://zeta-ai.io/*
// @run-at       document-start
// @grant        none
// ==/UserScript==

(${themeRuntime.toString()})(${JSON.stringify(A)},${JSON.stringify(O)},${JSON.stringify(C)},${F},${B},${JSON.stringify(BASE_THEME_URL)},false);
`;
}
...
function generateBookmarklet(){const A=accentPicker.value.toUpperCase(),O=otherPicker.value.toUpperCase(),C=bgPicker.value.toUpperCase(),F=getFontSize(),B=!!bubbleBorder?.checked,U=BASE_THEME_URL;return 'javascript:('+themeRuntime.toString()+')('+[A,O,C,F,B,U,true].map(v=>JSON.stringify(v)).join(',')+')';}

function applyFromURL(){
  const p=new URLSearchParams(location.search),a=norm(p.get('bubble')||''),o=norm(p.get('other')||''),b=norm(p.get('bg')||''),f=Number(p.get('fs')),br=p.get('border');
...
function render(){
  const accent=accentPicker.value.toUpperCase(),other=otherPicker.value.toUpperCase(),chat=bgPicker.value.toUpperCase(),fontPx=getFontSize(),borderOn=!!bubbleBorder?.checked;
...
accentPicker.addEventListener('input',render);otherPicker.addEventListener('input',render);bgPicker.addEventListener('input',render);fontSize.addEventListener('input',render);bubbleBorder?.addEventListener('input',render);
...
$('#resetTheme').addEventListener('click',()=>{fontSize.value='15';if(bubbleBorder)bubbleBorder.checked=false;setColors('#FEE500','#FFFFFF','#B2C7D9')});
