const BASE_THEME_VERSION = '3.50.20';
...
function themeRuntime(A, O, C, F, MB, MBC, MBW, OB, OBC, OBW, U, notify) {
  ...
  const custom=()=>{const hov=lum(A)>.55?mx(A,'#000000',.08):mx(A,'#FFFFFF',.1),surf=lum(C)<.28?mx(C,'#FFFFFF',.86):mx('#FFFFFF',C,.025),s2=mx(surf,C,.08),line=mx(surf,txt(surf),.1),sub=mx(txt(surf),C,.45),me=txt(A),ai=txt(O),aiAction=mx(ai,O,.34),aiSoft=mx(O,ai,.08),aiLine=mx(O,ai,.16),chatText=txt(C),chatSub=mx(chatText,C,.28),chatMuted=mx(chatText,C,.48),chatGlass=mx(C,chatText,.12),chatLine=mx(C,chatText,.20),userAction=mx(me,A,.28),userSoft=mx(A,me,.10),userLine=mx(A,me,.20);return '\n\n/* Zeta Theme Maker custom overrides */'
    +'\n:root{--kt-yellow:'+A+';--kt-yellow-hover:'+hov+';--kt-chat:'+C+';--kt-chat-text:'+chatText+';--kt-chat-sub:'+chatSub+';--kt-chat-muted:'+chatMuted+';--kt-chat-glass:'+chatGlass+';--kt-chat-line:'+chatLine+';--kt-white:'+surf+';--kt-soft:'+s2+';--kt-soft2:'+mx(s2,C,.08)+';--kt-text:'+txt(surf)+';--kt-sub:'+sub+';--kt-muted:'+mx(txt(surf),surf,.55)+';--kt-line:'+line+';--kt-user-dialogue:'+me+';--kt-user-action:'+userAction+';--kt-user-soft:'+userSoft+';--kt-user-line:'+userLine+';--kt-ai-dialogue:'+ai+';--kt-ai-action:'+aiAction+';--kt-ai-soft:'+aiSoft+';--kt-ai-line:'+aiLine+'}'
  ...
  (async()=>{try{const res=await fetch(U+'?custom='+Date.now(),{cache:'no-store'});if(!res.ok)throw Error('HTTP '+res.status);let s=await res.text(),t='  const CSS = `',i=s.indexOf(t);if(i<0)throw Error('CSS start');let b=i+t.length,e=s.indexOf('\n  `;',b);if(e<0)throw Error('CSS end');let css=s.slice(b,e).replace(/#[0-9a-fA-F]{6}\b/g,rc);const ac=h2r(A);css=css.replace(/rgba\(\s*254\s*,\s*229\s*,\s*0\s*,\s*([0-9.]+)\s*\)/gi,(_,a)=>'rgba('+ac.r+','+ac.g+','+ac.b+','+a+')').replace(/rgb\(\s*254\s*,\s*229\s*,\s*0\s*\)/gi,'rgb('+ac.r+','+ac.g+','+ac.b+')')+custom();s=s.slice(0,b)+css+s.slice(e);s=s.replace("const STYLE_ID = 'zeta-kakaotalk-theme-style';","const STYLE_ID = 'zeta-custom-theme-style';");(0,eval)(s);if(notify)alert('커스텀 테마가 적용되었습니다.\n새로고침하면 해제됩니다.')}catch(e){console.error('[Zeta Custom Theme]',e);if(notify)alert('테마 실행에 실패했습니다: '+e.message)}})();
}
...
function buildStandaloneScript() {
  ...
  const OB=!!otherBorder?.checked, OBC=(otherBorderPicker?.value||'#53636C').toUpperCase(), OBW=getBorderWidth(otherBorderWidth);
  return `// ==UserScript==
...
(${themeRuntime.toString()})(${JSON.stringify(A)},${JSON.stringify(O)},${JSON.stringify(C)},${F},${MB},${JSON.stringify(MBC)},${MBW},${OB},${JSON.stringify(OBC)},${OBW},${JSON.stringify(BASE_THEME_URL)},false);
`;
}
