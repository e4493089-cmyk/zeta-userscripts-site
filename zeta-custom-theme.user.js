...
(() => {
  'use strict';
  const BASE='https://raw.githubusercontent.com/e4493089-cmyk/zeta-userscripts/f0f66290ba5a6fc4f7ece330679917c1d08559c0/zeta-kakaotalk-theme.user.js';
  ...
  function sourceParams(){
    try{
      ...
      for(const u of urls){if(u&&/^https?:/i.test(u)){const p=new URL(u).searchParams;if(p.has('bubble')||p.has('other')||p.has('bg')||p.has('fs'))return p}}
    }catch(_){}
    return new URLSearchParams(location.search);
  }
  const p=sourceParams(),ACCENT=norm(p.get('bubble'))||'#FEE500',OTHER=norm(p.get('other'))||'#FFFFFF',CHAT=norm(p.get('bg'))||'#B2C7D9',FONT_SIZE=clamp(Number(p.get('fs'))||15,12,20);
